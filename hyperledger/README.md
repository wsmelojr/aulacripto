...
Esse script usa os arquivos [configtx.yaml](blockchain/configtx.yaml) e [crypto-config-ptb.yaml](blockchain/crypto-config-ptb.yaml) para criar os certificados MSP em uma pasta chamada (crypto-config). It also generates the genesis block file *ptb-genesis.block* and the channel configuration file *ptb-channel.tx*. Noticed that this script depends on the tools installed together with Fabric. The script *installFabric.sh* executed previously is expected to modify your $PATH variable and enable the direct execution of the Fabric tools. If this does not happen, try to fix the $PATH manually. The tools usually are located in the folder /$HOME/fabric_samples/bin.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMzUyNjExNjcsLTE5NzEyNjA3NV19
-->